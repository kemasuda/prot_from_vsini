...
datadir, nobs = "sim-2lognorm_1p1_0p1_0p6_0p1/", 50
datadir, nobs = "sim-loguni_m0p5_1/", 50

#%%
fname = glob.glob(datadir+"samples.csv")[0]
print (fname)
...
#%%
if ('loguni' in datadir and nobs == 50):
    dcat = dcat.iloc[nobs:nobs*2].reset_index(drop=True)
else:
    dcat = dcat.iloc[:nobs].reset_index(drop=True) # corresponds to an atypical realization for log-uniform distribution w/ and n=50
print (len(dcat))
...
if not null:
    if truth is not None:
        plt.plot(logpgrid, truth, '-', color='C1', lw=1, ls='dashed', label='input distribution')
    if "sim" in datadir:
